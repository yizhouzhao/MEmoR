...
class AddAMER(BaseModel):
    ...
    def initialize(self, config, device):
        ...
        self.enc_t = nn.Sequential(
            nn.Linear(D_t, D_e * 4),
            nn.ReLU(),
            nn.Linear(D_e * 4, int(1.5 * D_e)), # D_e = 128
        )

        self.enc_t_add = nn.Sequential(
            nn.Linear(24, D_e * 4),
            nn.ReLU(),
            nn.Linear(D_e * 4, int(0.5 * D_e)), # D_e = 128
        )      
    ...
    def forward(self, U_v, U_a, U_t, U_p, M_v, M_a, M_t, seq_lengths, target_loc, seg_len, n_c, U_t_add):
        # Encoders
        ...
        for i in range(M_v.shape[0]):
            ...
            inp_T = T_e_merge[i, : seq_lengths[i], :].reshape((n_c[i], seg_len[i], -1)).transpose(0, 1)
